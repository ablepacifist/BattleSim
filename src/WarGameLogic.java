...
public class WarGameLogic extends JFrame {
            ...
            case betweenTurns:
                if (command.equalsIgnoreCase("battle") || command.equalsIgnoreCase("s")) {
                    if (getBattle() != null) {
                        ...
                        if(checkForWin(getBattle().getA1())){
                            printToGui("congrats! "+ getBattle().getA2().getName()+ " has one the battle!");
                            currentState = State.NONE;
                        }else if(checkForWin(getBattle().getA2())){
                            printToGui("congrats! "+ getBattle().getA1().getName()+ " has one the battle!");
                            currentState = State.NONE;
                        }else{
                        initTakeTurn();
                        }
    ...
    public void takeTurn() {
        ...
        if (currentUnit == getBattle().getTurnOrder().getTopNode()) {
            //increase exhaustion:
            Set.NodeInterface next = getBattle().getTurnOrder().getTopNode().getLink();
            while(next != getBattle().getTurnOrder().getTopNode() && (next != getBattle().getTurnOrder().getLastNode())){
                next.getUnit().changeExhausted(.3);
                next = next.getLink();
            }
    ...
    // checks Range of current Unit and their targets and nulls any that are outside range
    ...
    // returns the distance in CELLS between units
...
//print helps
    ...
    //getters and setters between here and the gui
